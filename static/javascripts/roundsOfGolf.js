...
var coords, coords1, pos1, pos2, p1, p2, infoWindow
...
function updateCurveMarkers (model) {
  ...
  for (var i = 0; i < model[0].features.length; i++) {
    if (model[0].features[i].geometry.type === 'LineString') {
      ...
      var m = new google.maps.Point(e.x / 2, e.y / 2) // midpoint
      var o = new google.maps.Point(e.y, -e.x) // orthogonal
      var c = new google.maps.Point(m.x + curvature * o.x, m.y + curvature * o.y) // curve control point
