...
module.exports = function(io) {
   ...
   * About
   */
  router.get(config.routes.about, [util.requireAuthentication], function(
    req,
    res
  ) {
   ...
   * Contact Page
   */
  router.get(config.routes.contact, [util.requireAuthentication], function(
    req,
    res
  ) {
   ...
   * Main Page
   */
  router.get(config.routes.mainPage, [util.requireAuthentication], function(
    req,
    res
  ) {
    var data = { title: "Golf Test Routines" };
   ...
   */
  router.get(
    config.routes.nearbyGolfCourses,
    [util.requireAuthentication],
    function(req, res) {
      var data = {
        title: "Golf Test Routines",
        pageName: "Nearby Golf Courses Page"
      };
        ...
        r.connect(
          ...
          function(err, conn) {
            ...
            r.db(config.rethinkdb.db)
              .table(config.rethinkdb.tables.nearbyGolfCourses)
                ...
                if (err) {
                  ...
                  cursor.toArray(function(err, nearbyGolfCoursesData) {
                    if (err) {
                      ...
                      io.emit(
                        "loadNearbyGolfCoursesData",
                        nearbyGolfCoursesData
                      );
                    }
                  });
                }
   ...
   * Get Rounds Of Golf
   */
  router.get(config.routes.roundsOfGolf, [util.requireAuthentication], function(
    req,
    res
  ) {
    var data = {
      title: "Golf Test Routines",
      pageName: "Get Rounds of Golf Page"
    };
    ...
    io.on("connection", function() {
      r.connect(
        ...
        function(err, conn) {
          ...
          r.db(config.rethinkdb.db)
            .table(config.rethinkdb.tables.roundOfGolf)
            .run(connection, function(err, cursor) {
              if (err) {
                console.log(
                  "[ERROR] %s:%s \n%s",
   ...
   */
  router.get(
    config.routes.findAllMyRounds,
    [util.requireAuthentication],
    function(req, res) {
      var data = {
        title: "Golf Test Routines",
        pageName: "Find All My Rounds Page"
      };
   ...
   */
  router.get(config.routes.addMyRound, [util.requireAuthentication], function(
    req,
    res
  ) {
   ...
   */
  router.get(
    config.routes.findMyRoundById,
    [util.requireAuthentication],
    function(req, res) {
      var data = {
        title: "Golf Test Routines",
        pageName: "Find My Round By Id Page"
      };
   ...
   */
  router.get(
    config.routes.deleteMyRound,
    [util.requireAuthentication],
    function(req, res) {
      var data = {
        title: "Golf Test Routines",
        pageName: "Delete My Round Page"
      };
   ...
   */
  router.get(
    config.routes.courseScorecards,
    [util.requireAuthentication],
    function(req, res) {
      var data = {
        title: "Golf Test Routines",
        pageName: "Course Scorecards Page"
      };
   ...
   */
  router.get(
    config.routes.membershipRelationshipManager,
    [util.requireAuthentication],
    function(req, res) {
      var data = {
        title: "Golf Test Routines",
        pageName: "Membership Relationship Manager Page"
      };
   ...
   * CinemaTest Page
   */
  router.get(config.routes.cinema, [util.requireAuthentication], function(
    req,
    res
  ) {
    var data = {
      title: "Cinema Test",
      pageName: "Cinema Test Page Using Firebase"
    };
   ...
   */
  router.get(
    config.routes.addGCCompetitions,
    [util.requireAuthentication],
    function(req, res) {
      var googleCalendar = new gcal.GoogleCalendar(
        config.google.googleOAuth2AccessToken
      );
      ...
      var addEventBody = {
        status: "confirmed",
        htmlLink:
          "https://www.google.com/calendar/event?eid=b28zdWNyYmoyOGlwaTluYmlvYjJjYzg5dDQgYmRzNjA1MkBt",
        ...
        location:
          "Clandeboye Golf Club, 51 Tower Rd, Bangor, Newtownards BT23, UK",
        creator: {
      ...
      googleCalendar.events.insert(config.calendarId, addEventBody, function(
        addEventError,
        addEventResponse
      ) {
        ...
        if (addEventError) {
          ...
          var data = {
            title: "Golf Test Routines",
            pageName: "Add google-calendar Competition Page"
          };
          res.render("about", data);
        }
      });
