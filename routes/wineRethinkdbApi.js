/**
 * Created by briansmith on 26/04/2016.
 */
...
exports.setupDB = function (dbConfig, connection) {
  ...
  r.dbCreate(dbConfig.db).run(connection, function (err) {
    if (err) {
      debug('[ERROR] %s:%s\n%s', err.name, err.msg, err.message)
      ...
      r.db(dbConfig.db).tableCreate(dbConfig.table).run(connection, function (err) {
        if (err) {
          debug('[ERROR] %s:%s\n%s', err.name, err.msg, err.message)
